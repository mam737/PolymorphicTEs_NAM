...
# export PATH=/home/brandvai/mmunasin/Software/bedtools2/bin/:$PATH
library(bedtoolsr) 
library(ggpubr)
library(grid)
set.seed(1994)
...
outfile_name <- paste('/home/brandvai/mmunasin/TE_Intron/store_data/filtered_TE_anno/summary_files/',lineage,'_filtering_summary_outputs.txt',sep='')
...
dir.create(paste('/home/brandvai/mmunasin/TE_Intron/store_data/filtered_TE_anno/',lineage,sep=''))

for (i in unique(final_TE_full$chr)) {
	chr_specific <- final_TE_full %>% subset(chr==i)
	file_path <- paste('/home/brandvai/mmunasin/TE_Intron/store_data/filtered_TE_anno/',lineage,'/',lineage,'_',i,'_EDTA_filtered.bed',sep='')
